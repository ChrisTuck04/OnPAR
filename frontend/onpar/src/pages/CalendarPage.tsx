...
const Journal = ({ onCloseJournal, currentEmotion, userID }: JournalProps) => {
  ...
  const resetEmotionFieldsIfNewDay = async () => {
    try {

      const response = await readEmotions(startOfDay(new Date()), endOfDay(new Date()))
      console.log("flag 1")
      const emotionArrLength = response.emotions.length
      console.log("flag 2")

      if(emotionArrLength > 0) {
        const existingEmotion = response.emotions[0];
        setEmotion(existingEmotion);
        setTitle(existingEmotion.title || "");
        setLeftText(existingEmotion.leftContent || "");
        setRightText(existingEmotion.rightContent || "");
        setSharedEmails(existingEmotion.sharedEmails || []);  
        return
      }
      ...
      const error = err as AxiosError<{error : string}>;
      console.error("Error retrieving emotions array", error.response?.data?.error || "Unknown error occurred.");
    }
  }

  useEffect(() => {
    resetEmotionFieldsIfNewDay()
  }, [])
  ...
  const checkIfEmotionExists = async () => {
    try {
      ...
      if(emotionArrLength > 0)
        return response.emotions[0]

      else {
        console.log("No emoition for current day yet, proceeding to create emotion for the day")
        return null
      }
  ...
  const updateEmotionObject = async (e : React.MouseEvent<HTMLButtonElement>, emotionId : string, currentEmotion : Emotions) => {
    try {

      console.log("updating emotion")
      const response = await updateEmotions(emotionId, currentEmotion)
      console.log("updated emotion successfully!", response)

      onCloseJournal(e)
    } catch(err: unknown) {
      const error = err as AxiosError<{error : string}>;
      console.error("Error updating emotion object:", error.response?.data?.error || "Unknown error occurred.");
      alert(`Failed to update emotion object: ${error.response?.data?.error || "Please try again."}`);
    }
  }
   ...
   const emotionCreationWrapper = async (e : React.MouseEvent<HTMLButtonElement>) => {
    ...
    const possibleEmotionObject = await checkIfEmotionExists()

    if(possibleEmotionObject === null)
      await createEmotionObject(e, updatedEmotion)
    else {
      await updateEmotionObject(e, possibleEmotionObject._id, updatedEmotion)
    }
  }
